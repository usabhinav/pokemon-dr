...
class PokeballPlayerSendOutAnimation < PokeBattle_Animation
  ...
  def createProcesses
    ...
    # CHANGED: Make sprite green if Zygarde
    col = Color.new(-255, 255, -255) if isConst?(@battler.pokemon.species,PBSpecies,:ZYGARDE)
    col.alpha = 255
    ...
    # CHANGED: Remove ball bitmap if battler is Zygarde
    ballType = -1 if isConst?(@battler.pokemon.species,PBSpecies,:ZYGARDE)
    # CHANGED: Remove ball bitmap if battler is Lucario from ship event
    ballType = -1 if $game_switches[74] && isConst?(@battler.pokemon.species,PBSpecies,:LUCARIO)
