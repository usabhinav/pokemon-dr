class PokeBattle_Scene
  ...
  def pbCommandMenu(idxBattler,firstAction)
    ...
    recruitOn = @battle.pbCanRecruit? # CHANGED: recruitOn
    cmds = [
    ...
    # CHANGED: Made mode a local variable
    mode = (recruitOn) ? 5 : (shadowTrainer) ? 2 : (firstAction) ? 0 : 1
    # CHANGED: Added extra command for recruit
    cmds.push(_INTL("Recruit")) if mode == 5
    ret = pbCommandMenuEx(idxBattler,cmds,mode)
    ret = 5 if ret==4 && recruitOn       # CHANGED: Convert "Run" to "Recruit"
  ...
  def pbCommandMenuEx(idxBattler,texts,mode=0)
    ...
    loop do
      oldIndex = cw.index
      pbUpdate(cw)
      # CHANGED: Handle arrow key input different for Recruit mode
      if mode == 5
        # Arrays: index = current cw.index, value = next cw.index
        left_pressed = [4, 0, 3, 2, 1]
        right_pressed = [1, 4, 3, 2, 0]
        up_pressed = [2, 3, 0, 1, 3]
        down_pressed = [2, 3, 0, 1, 3]
        # Update selected command
        if Input.trigger?(Input::LEFT)
          cw.index = left_pressed[cw.index]
        elsif Input.trigger?(Input::RIGHT)
          cw.index = right_pressed[cw.index]
        elsif Input.trigger?(Input::UP)
          cw.index = up_pressed[cw.index]
        elsif Input.trigger?(Input::DOWN)
          cw.index = down_pressed[cw.index]
        end
      else
        # Update selected command
        if Input.trigger?(Input::LEFT)
      ...
      if Input.trigger?(Input::C)                 # Confirm choice
        ...
        ret = -2
        break
      elsif Input.trigger?(Input::CTRL)           # CHANGED: Z Transform
        @battle.pbTransform
        break
      end
    end
