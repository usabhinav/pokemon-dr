...
class CommandMenuDisplay < BattleMenuBase
  ...
  MODES = [
     ...
     [10,11,13,14,12]   # CHANGED: 5 = Use "Recruit"
  ]

  def initialize(viewport,z)
    super(viewport)
    @viewport = viewport # CHANGED: Store viewport
    self.x = 0
  ...
  def refreshButtons
    return if !USE_GRAPHICS
    viewport = @viewport
    # CHANGED: Place buttons differently for recruit mode
    @buttons.each do |button|
      button.dispose
    end
    if @mode == 5
      x_values = [252, 340, 252, 382, 424]
      y_values = [294, 294, 340, 340, 294]
      width_values = [88, 84, 130, 130, 88]
      # Create action buttons
      @buttons = Array.new(5) do |i|   # 5 command options, therefore 5 buttons
        button = SpriteWrapper.new(viewport)
        button.bitmap = @buttonBitmap.bitmap
        button.x      = x_values[i]
        button.y      = y_values[i]
        button.src_rect.width  = width_values[i]
        button.src_rect.height = BUTTON_HEIGHT
