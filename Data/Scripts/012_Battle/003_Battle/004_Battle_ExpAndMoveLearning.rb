class PokeBattle_Battle
    ...
    @battlers.each do |b|
      ...
      if numPartic>0 || expShare.length>0 || expAll
        ...
        eachInTeam(0,0) do |pkmn,i|
          ...
          # CHANGED: Zygarde cannot gain exp from opponents lower than 4 levels below his/her own
          next if pkmn.species == PBSpecies::ZYGARDE && pkmn.level >= b.level + 4
          # CHANGED: Disable exp gains in Underdome for Pokemon higher than current level cap
          next if $game_switches[82] && pkmn.level > pbGet(39)
          pbGainEVsOne(i,b)
      ...
      # CHANGED: Gives Zygarde exp even if not in battle
      containsZygarde = false
      for poke in b.participants
        if isConst?(@party1[poke].species,PBSpecies,:ZYGARDE)
          containsZygarde = true
          break
        end
      end
      if $game_switches[Z_TRANSFORM_SWITCH] && !containsZygarde &&
         $Trainer.zygarde.level <= b.level + 4
        @party1.push($Trainer.zygarde)
        pbGainEVsOne(@party1.length-1,b)
        pbGainExpOne(@party1.length-1,b,numPartic,expShare,expAll)
        @party1.delete_at(@party1.length-1)
      end
  ...
  def pbGainExpOne(idxParty,defeatedBattler,numPartic,expShare,expAll,showMessages=true)
    ...
    # CHANGED: Includes Zygarde as "participant"
    elsif (isPartic || pkmn.isSpecies?(:ZYGARDE))   # Participated in battle, no Exp Shares held by anyone
    ...
    exp = i if i>=0
    # CHANGED: Give reduced exp to Zygarde in battle
    if pkmn.isSpecies?(:ZYGARDE)
      if isPartic # If in battle and participated against foe
        exp = (exp*0.5).floor
      else
        exp = (exp*0.25).floor
      end
    ...
    loop do   # For each level gained in turn...
      ...
      battler.pbUpdate(false) if battler
      @scene.pbRefreshOne(battler.index) if battler
      pbMEPlay("Level Up Sound.mp3") # CHANGED: Added sound effect for leveling up
