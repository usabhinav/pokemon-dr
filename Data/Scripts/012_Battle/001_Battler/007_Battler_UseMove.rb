        ...
        if !success
          if i==0 && targets.length>0
            ...
            targets.each do |t|
              ...
              # CHANGED: Recruit chance on critical hit
              if @battle.pbCanRecruit? && t.damageState.critical && t.opposes? && rand(100) < 15
                @battle.pbDisplay(_INTL("{1} was knocked down by the hit, and you captured it swiftly!", t.pbThis))
  ...
  def pbProcessMoveHit(move,user,targets,hitNum,skipAccuracyCheck)
    ...
    if move.pbDamagingMove?
      ...
      targets.each do |b|
        ...
        @battle.pbRestartIfKilled(b.index, b.hp, b.pokemon.species)
      end
