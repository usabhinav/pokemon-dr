...
#===============================================================================
# CHANGED: Compile disguises
#===============================================================================
def pbCompileDisguises
  # Make sure to keep code for creating constants for PBDisguises, and at the
  # same time, follow Marin's tutorial on storing the records in a $ global
  # variable (look at PItem_Items)
  records   = []
  code = "class PBDisguises\r\n"
  pbCompilerEachPreppedLine("PBS/disguises.txt"){|line,lineno|
    record = pbGetCsvRecord(line,lineno,[0,"unsuvs"])
    # Name, Outfit ID, Step Count, Description
    records[record[0]] = [record[2], record[3], record[4], record[5]]
    code += "#{record[1]}=#{record[0]}\r\n"
  }
  code += "end\r\n"
  eval(code)
  pbAddScript(code,"PBDisguises")
  save_data(records, "Data/disguises.dat")
end
...
def pbCompileQuests
  ...
  pbCompilerEachCommentedLine("PBS/quests.txt") { |line,lineno|
    if line[/^\s*\[\s*(.+)\s*\]\s*$/]
      ...
      section = pbGetCsvRecord($~[1],lineno,[0,"u"])
      if questindex + 2 != section
      ...
      case settingname
      when "Name"
        quests[questindex][0] = record
      when "InternalName"
        code += "#{record} = #{questindex + 1}\r\n"
      when "Summary"
        quests[questindex][1] = record
      when "Completed"
