...
# CHANGED: Item handler for Pocket App
ItemHandlers::UseFromBag.add(:POCKETAPP,proc { |item|
  pbCallPocketApp
  next 1
})
...
# CHANGED: Zygarde Cell Item Handler
ItemHandlers::UseOnPokemon.add(:ZYGARDECELL,proc { |item,pkmn,scene|
  if pkmn.level>=PBExperience.maxLevel || pkmn.shadowPokemon? || !pkmn.isSpecies?(:ZYGARDE)
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  pbChangeLevel(pkmn,pkmn.level+1,scene)
