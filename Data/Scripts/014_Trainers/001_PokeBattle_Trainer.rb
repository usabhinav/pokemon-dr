class PokeBattle_Trainer
  ...
  def initialize(name,trainertype)
    ...
    @money             = INITIAL_MONEY
    @party             = []
    @lucario           = false # CHANGED: Initialized lucario
    # CHANGED: Initialized zygarde
    if trainertype <= 1
      @zygarde         = PokeBattle_Pokemon.new(PBSpecies::ZYGARDE, trainertype == 1 ? 15 : 13, self)
      @zygarde.pbLearnMove(PBMoves::DRAGONBREATH)
      @zygarde.pbLearnMove(PBMoves::EXTREMESPEED)
      @zygarde.pbLearnMove(PBMoves::THOUSANDWAVES)
      @zygarde.pbLearnMove(PBMoves::BITE)
      @zygarde.setNature((trainertype == 1) ? (:ADAMANT) : (:JOLLY))
    else
      @zygarde         = nil
    end
