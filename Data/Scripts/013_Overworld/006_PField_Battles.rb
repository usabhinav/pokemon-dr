...
class PokemonTemp
  ...
  def recordBattleRule(rule,var=nil)
    ...
    # CHANGED: Battle rule for "no recruiting"
    when "cannotrecruit";          rules["cannotRecruit"]  = true
    else
...
#    when PBEnvironment::Rock;                                   base = "rock"
  ...
  # CHANGED: Whether player can recruit or not
  battle.canRecruit = false if !battleRules["cannotRecruit"].nil?
end
...
def pbCanDoubleBattle?
  ...
  # CHANGED: Allow only one Pokemon (Z-Transform if necessary)
  return $Trainer.ablePokemonCount >= 1 || pbCanTransform?
end
...
def pbWildBattleCore(*args)
  ...
  # CHANGED: Allow Z-Transform if no able Pokemon
  if ($Trainer.ablePokemonCount==0 && !pbCanTransform?) || 
  ...
  # CHANGED: Put Zygarde if can transform
  if pbCanTransform?
    if $Trainer.ablePokemonCount == 0
      playerParty.push($Trainer.zygarde)
    else
      totalAbleCount = $Trainer.ablePokemonCount + ($PokemonGlobal.partner ? 1 : 0)
      if totalAbleCount < foeParty.length && pbConfirmMessage(_INTL("You have less able participants than your opponents. Z-Transform?"))
        playerParty.push($Trainer.zygarde)
      end
    end
  end
...
def pbTrainerBattleCore(*args)
  ...
  if pbCanTransform?
    if $Trainer.ablePokemonCount == 0
      ...
      totalAbleCount = $Trainer.ablePokemonCount + ($PokemonGlobal.partner ? 1 : 0)
      if totalAbleCount < foePartyStarts.length && pbConfirmMessage(_INTL("You have less able participants than your opponents. Z-Transform?"))
        playerParty.push($Trainer.zygarde)
      end
    end
  end
...
def pbTrainerBattle(trainerID, trainerName, endSpeech=nil,
  ...
  setBattleRule("outcomeVar",outcomeVar) if outcomeVar!=1
  setBattleRule("canLose") if canLose
  # CHANGED: Allow double battle if have partner registered
  setBattleRule("double") if doubleBattle || $PokemonTemp.waitingTrainer || $PokemonGlobal.partner
  # Perform the battle
...
def pbAfterBattle(decision,canLose)
  ...
  if $PokemonGlobal.partner
    # CHANGED: Pokemon are not healed after double battle
    # pbHealAll
  ...
  if decision==2 || decision==5   # if loss or draw
    if canLose
      ...
      $Trainer.zygarde.heal # CHANGED: Heal Zygarde if canlose
...
def pbPickup(pkmn)
  ...
  # CHANGED: Commented out certain revive items
