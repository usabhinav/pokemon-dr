  ...
  if $PokemonGlobal.stepcount%4==0 && POISON_IN_FIELD
    ...
    for i in $Trainer.ablePokemonParty
      if i.status==PBStatuses::POISON && !isConst?(i.ability,PBAbilities,:IMMUNITY)
        ...
        if pbAllFainted
          handled[0] = true
          # CHANGED: Don't start over if Pokemon die in field
          pbCheckAllFainted if $Trainer.party.length > 5
        end
      end
    end
  end
...
def pbBattleOnStepTaken(repel=false)
  # CHANGED: Allow Z-Transform at start of battle if no able Pokemon
  return if $Trainer.ablePokemonCount==0 && !pbCanTransform?
  ...
  if $PokemonEncounters.pbCanEncounter?(encounter,repel)
    if !$PokemonTemp.forceSingleBattle && !pbInSafari? && ($PokemonGlobal.partner ||
       # CHANGED: Removed $Trainer.ablePokemonCount>1 check
       (PBTerrain.isDoubleWildBattle?(pbGetTerrainTag) && rand(100)<30))
...
def pbFishing(hasEncounter,rodType=1)
  ...
  loop do
    ...
    if pbWaitMessage(msgWindow,time)
      ...
      $Trainer.outfit = oldoutfit # CHANGED: Zygarde fishing
      break
    end
...
Events.onAction+=proc{|sender,e|
  ...
  return if !PBTerrain.isFishable?(Kernel.pbFacingTerrainTag)
  x = $game_player.x
  y = $game_player.y
  case $game_player.direction
  when 2; y += 1
  ...
  for event in $game_map.events.values
    return if event.tile_id >= 0 and event != $game_player and event.x == x and event.y == y and not event.through
  end
  pbZygardeFish
}
