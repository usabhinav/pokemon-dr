      ...
      loop do
        ...
        if selected==nil
          ...
          when 0
            ...
            # CHANGED: Quits box if Trainer has not enough money
            if $Trainer.money < POKE_STORAGE_FEE
              pbMessage(_INTL("You don't have enough money for any more transactions!"))
              break
            end
...
### DEPOSIT ####################################################################
  ...
  def pbWithdraw(selected,heldpoke)
    ...
    # CHANGED: Don't allow player to have 6 unusable Pokemon at any given time
    pkmn=@storage[selected[0], selected[1]]
    if @storage.party.nitems>=5 && pbAbleCount<=0 && (pkmn.hp<=0 || pkmn.isEgg?)
      pbDisplay(_INTL("You can't have 6 unusable Pokemon!"))
      return false
    end
    # CHANGED: Asks user for confirmation, charges fee if yes, breaks if no
    if pbConfirmMessage(_INTL("\\GPYou will be charged ${1}. Is that OK?", POKE_STORAGE_FEE))
      $Trainer.money -= POKE_STORAGE_FEE
      pbMessage(_INTL("{1} paid ${2}.", $Trainer.name, POKE_STORAGE_FEE))
    else
      @scene.pbRefresh
      return false
    end
  ...
  def pbStore(selected,heldpoke)
    ...
    # CHANGED: Allows player to deposit all Pokemon if Z transform is enabled
    if pbAbleCount<=1 && pbAble?(@storage[box,index]) && !heldpoke && !$game_switches[Z_TRANSFORM_SWITCH]
      pbPlayBuzzerSE
      ...
      loop do
        ...
        if destbox>=0
          ...
          if heldpoke || selected[0]==-1
            ...
            # CHANGED: PC no longer heals Pokemon
            # p.heal
          end
