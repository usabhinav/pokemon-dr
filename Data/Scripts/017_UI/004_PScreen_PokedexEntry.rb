...
#        @region = pbDexNames[i][1] if pbDexNames[i].is_a?(Array)
  ...
  def pbUpdateDummyPokemon
    ...
    # CHANGED: Grey out icon if only seen (not owned)
    seen = $Trainer.hasSeen?(@species) && !$Trainer.hasOwned?(@species)
    grey = seen ? 255 : 0
    @sprites["infosprite"].tone = Tone.new(0, 0, 0, grey)
    @sprites["formfront"].tone = Tone.new(0, 0, 0, grey) if @sprites["formfront"]
    @sprites["formback"].tone = Tone.new(0, 0, 0, grey) if @sprites["formback"]
    @sprites["formicon"].tone = Tone.new(0, 0, 0, grey) if @sprites["formicon"]
  end
