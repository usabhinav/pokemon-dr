...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      cmdRead     = -1
      cmdUse      = -1
      cmdZygarde  = -1 # CHANGED: Use on Zygarde
      cmdRegister = -1
      cmdGive     = -1
      cmdToss     = -1
      cmdDebug    = -1
      ...
      # CHANGED: Shows Zygarde option
      if pbCanUseOnPokemon?(item) && $game_switches[Z_TRANSFORM_SWITCH]
        commands[cmdZygarde = commands.length]    = _INTL("Zygarde")
      end
      ...
      if cmdRead>=0 && command==cmdRead   # Read mail
        ...
        @scene.pbRefresh
        next
      # CHANGED: Zygarde option
      elsif cmdZygarde>=0 && command==cmdZygarde   # Use item on Zygarde
        oldparty = $Trainer.party
        $Trainer.party = [$Trainer.zygarde]
        ret = pbUseItem(@bag,item,@scene)
        # ret: 0=Item wasn't used; 1=Item used; 2=Close Bag to use in field
  ...
  def pbWithdrawItemScreen
    ...
    loop do
      ...
      if @bag.pbCanStore?(item,qty)
        # CHANGED: Fee for item withdrawal (quantity taken into account)
        if $Trainer.money < ITEM_STORAGE_FEE * qty
          pbDisplay(_INTL("You can't afford that!"))
          next
        end
        # CHANGED: Confirm item withdrawal
        if !pbConfirmMessage(_INTL("\\GYou will be charged ${1}. Is that OK?", ITEM_STORAGE_FEE * qty))
          next
        end
        ...
        # CHANGED: Remove money
        $Trainer.money -= ITEM_STORAGE_FEE * qty
        pbDisplay(_INTL("{1} paid ${2}.", $Trainer.name, ITEM_STORAGE_FEE * qty))
        # CHANGED: Quit box if Trainer money insufficient
        if $Trainer.money < ITEM_STORAGE_FEE
          pbDisplay(_INTL("You don't have enough money for any more transactions!"))
          break
        end
      else
    ...
    loop do
      ...
      if qty>0
        if !storage.pbCanStore?(item,qty)
          ...
          # CHANGED: Fee for item deposit (quantity taken into account)
          if $Trainer.money < ITEM_STORAGE_FEE * qty
            pbDisplay(_INTL("You can't afford that!"))
            next
          end
          # CHANGED: Confirm item deposit
          if !pbConfirmMessage(_INTL("\\GYou will be charged ${1}. Is that OK?", ITEM_STORAGE_FEE * qty))
            next
          end
