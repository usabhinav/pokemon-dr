...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    pagename = [_INTL("INFO"),
                _INTL("TRAINER MEMO"),
                _INTL("SKILLS"),
                _INTL("EVs/IVs"), # CHANGED: Added EVs/IVs page
                _INTL("MOVES"),
    ...
    when 4; drawPageFour
    when 5; drawPageFive
    when 6; drawPageSix # CHANGED: Added page 6
    end
  end
  ...
  # CHANGED: Added page for EVs/IVs
  def drawPageFour
   overlay = @sprites["overlay"].bitmap
    base   = Color.new(248,248,248)
    shadow = Color.new(104,104,104)
    ...
    textpos = [
       ...
       # CHANGED: X/Y values to match HP coordinates in page 3
       [sprintf("%d/%d",@pokemon.ev[0],@pokemon.iv[0]),462,76,1,Color.new(64,64,64),Color.new(176,176,176)],
  ...
  # CHANGED: Made moves page 5 instead of 4
  def drawPageFive
    overlay = @sprites["overlay"].bitmap
  ...
  # CHANGED: Made ribbons page 6 instead of 5
  def drawPageSix
    overlay = @sprites["overlay"].bitmap
    @sprites["uparrow"].visible   = false
    @sprites["downarrow"].visible = false
  ...
  def pbScene
    ...
    loop do
      ...
      if Input.trigger?(Input::A)
        ...
        if @page==5 # CHANGED: Move selection for page 5
          pbPlayDecisionSE
          pbMoveSelection
          dorefresh = true
        elsif @page==6 # CHANGED: Ribbon selection for page 6
          pbPlayDecisionSE
          pbRibbonSelection
          dorefresh = true
        elsif !@inbattle
        ...
        @page -= 1
        @page = 1 if @page<1
        @page = 6 if @page>6 # CHANGED: Increased max page to 6
