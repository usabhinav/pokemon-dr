# CHANGED: Commented out any lines regarding @pic2 (flashing "Press Enter" picture)
# CHANGED: Splash picture is now a GifSprite, code has been edited to accomodate change
class IntroEventScene < EventScene
  ...
  def openSplash(_scene,*args)
    onUpdate.clear
    onCTrigger.clear
    # CHANGED: pic GifSprite
    @pic = GifSprite.new(@splash)
    @pic.name = "Graphics/Titles/"+@splash
    # @pic.moveOpacity(0,FADE_TICKS,255)
    @pic.opacity = 255 # CHANGED: opacity method
  ...
  def closeSplash(scene,*args)
    ...
    # Fade out
    # @pic.moveOpacity(0,FADE_TICKS,0)
    @pic.opacity = 0 # CHANGED: opacity method
    # @pic2.clearProcesses
